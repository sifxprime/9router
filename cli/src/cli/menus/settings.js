...
/**
 * Reset dashboard password to default via API.
 * Uses the CLI token to authenticate with the local server.
 */
async function resetPassword() {
  ...
  if (result.success) {
    showStatus(result.data?.message || `Password reset. Default: ${DEFAULT_PASSWORD}`, "success");
  } else {
    showStatus(`Failed: ${result.error}`, "error");
  }
  await pause();
