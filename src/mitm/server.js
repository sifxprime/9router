...
const handlers = {
  ...
  claude: require("./handlers/claude"),
};
...
async function resolveTargetIP(hostname) {
  // Node v26: dns.resolve4() throws ENOTFOUND on raw IPv4 — skip DNS for IPs
...
async function passthrough(req, res, bodyBuffer, onResponse) {
  ...
  log(`[passthrough] ${req.method} ${originalHost}${req.url} → ${targetHost}`);
  const dumper = ENABLE_FILE_LOG ? createResponseDumper(req, "passthrough") : null;

  const tool = getToolForHost(req.headers.host);
  // Only rewrite version metadata on chat endpoints — auth endpoints (fetchUserInfo, etc.)
  // reject the old version string (1.23.2) and loop forever on "Authenticating..."
...
async function passthroughHttp2(req, res, bodyBuffer, headers, targetHost, onResponse, dumper) {
  ...
  return new Promise((resolve) => {
    ...
    stream.once("response", (responseHeaders) => {
      ...
      log(`[h2-response] ${req.method} ${targetHost}${req.url} → ${status}`);
...
const server = https.createServer(sslOptions, async (req, res) => {
  try {
    ...
    // Anti-loop: skip requests from kRouter
