...
# cs45 -> cc/claude-sonnet-4-5-20250929`,
  ...
  "codewhale": {
    ...
    color: "#4D6BFE",
    description: "CodeWhale Terminal Coding Agent (Rust TUI)",
    ...
    notes: [
      { type: "info", text: "CodeWhale uses ~/.codewhale/config.toml for configuration (legacy ~/.deepseek/config.toml is still read on first launch). 9Router will update the provider to 'openai' mode with your base_url, api_key, and model." },
      { type: "warning", text: "Config path: Linux/macOS ~/.codewhale/config.toml (legacy ~/.deepseek/config.toml) • Windows %USERPROFILE%\\.codewhale\\config.toml" },
    ],
  },
  jcode: {
    id: "jcode",
